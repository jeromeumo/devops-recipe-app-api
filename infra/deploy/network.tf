...
####################################################################
# Endpoints to allow access to ECR, Cloudwatch and systems manager #
####################################################################
...
resource "aws_vpc_endpoint" "cloudwatch_logs" {
  vpc_id              = aws_vpc.main.id
  service_name        = "com.amazonaws.${data.aws_region.current.name}.log"
  vpc_endpoint_type   = "Interface"
  private_dns_enabled = true
  ...
  tags = {
    Name = "${local.prefix}-cloudwatch-logs-endpoint"
  }
}
